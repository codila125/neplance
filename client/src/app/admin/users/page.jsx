    ...
    <main className="section" style={{ backgroundColor: "var(--color-bg-page)" }}>
      <div className="container">
        <div className="card" style={{ padding: "var(--space-8)" }}>
          ...
          <form method="get" style={{ display: "flex", gap: "var(--space-3)", marginBottom: "var(--space-6)" }}>
            <input name="search" className="form-input" placeholder="Search by name or email" defaultValue={search} />
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u._id} style={{ borderBottom: "1px solid var(--color-border-light)", verticalAlign: "middle" }}>
                    <td style={{ padding: "var(--space-3)" }}>{u.name}</td>
                    <td style={{ padding: "var(--space-3)" }}>{u.email}</td>
                    <td style={{ padding: "var(--space-3)" }}>{Array.isArray(u.role) ? u.role.join(", ") : u.role}</td>
                  </tr>
                ))}
                {users.length === 0 && (
                  <tr>
                    <td colSpan={3} style={{ padding: "var(--space-4)", textAlign: "center" }}>No users found</td>
                  </tr>
                )}
              </tbody>
            </table>
